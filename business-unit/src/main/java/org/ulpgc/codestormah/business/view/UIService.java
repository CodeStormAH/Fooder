...
public class UIService {
    ...
    public void start() {
        ...
        app.get("/api/products/{category}", ctx ->
                ctx.json(productStore.getProductsByCategory(ctx.pathParam("category"))));

        app.get("/api/products/{category}/cheapest", ctx ->
                ctx.json(productStore.getCheapestProduct(ctx.pathParam("category"))));

        app.get("/api/products/{category}/expensive", ctx ->
                ctx.json(productStore.getMostExpensiveProduct(ctx.pathParam("category"))));

        app.get("/api/recommendation/{category}", ctx -> {
            Recommendation rec = recommendationStore.get(ctx.pathParam("category"));
            if (rec == null) {
                ctx.status(404).result("Sin recomendación para esa categoría");
            } else {
                ctx.json(rec);
            }
        });

        app.get("/api/history/{source}/{id}", ctx ->
                ctx.json(productStore.getProductHistory(
                        ctx.pathParam("id"),
                        ctx.pathParam("source")
                ))
        );
